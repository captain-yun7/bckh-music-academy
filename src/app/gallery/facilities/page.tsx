...
export default function FacilitiesGalleryPage() {
  const [facilities, setFacilities] = useState<GalleryImage[]>([]);
  ...
  if (isLoading) {
    return (
      <SubPageLayout title="시설 사진" subtitle="최신 장비와 쾌적한 환경에서 음악에만 집중할 수 있습니다">
    ...
    <>
      ...
      {facilities[currentIndex] && (
        <ImageLightbox
          ...
          imageSrc={facilities[currentIndex].imageUrl}
          imageAlt={facilities[currentIndex].title || `시설 ${currentIndex + 1}`}
          title={facilities[currentIndex].title || undefined}
          description={facilities[currentIndex].description || undefined}
          onPrev={goToPrev}
          onNext={goToNext}
          showNavigation={true}
        />
