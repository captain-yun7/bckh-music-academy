...
export default function GalleryPage() {
  ...
  useEffect(() => {
    const fetchCoverImages = async () => {
      try {
        const [perfRes, facRes] = await Promise.all([
          fetch('/api/gallery?category=PERFORMANCE'),
          fetch('/api/gallery?category=FACILITY'),
        ]);
        const perfData: GalleryImage[] = perfRes.ok ? await perfRes.json() : [];
        const facData: GalleryImage[] = facRes.ok ? await facRes.json() : [];

        setCoverImages({
          PERFORMANCE: perfData[0]?.imageUrl || '',
          FACILITY: facData[0]?.imageUrl || '',
        });
      } catch (error) {
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '32px' }}>
            {galleryCategories.map((gallery) => {
              const imageUrl = coverImages[gallery.category];
              return (
                <Link
                ...
                >
                  <div style={{ position: 'relative', aspectRatio: '16/10' }}>
                    {imageUrl ? (
                      <Image
                        src={imageUrl.includes('cloudinary.com') ? imagePresets.galleryThumb(imageUrl) : imageUrl}
                        alt={gallery.title}
                        fill
                        style={{ objectFit: 'cover', transition: 'transform 0.3s ease' }}
                        sizes="(max-width: 768px) 100vw, 50vw"
                        placeholder={imageUrl.includes('cloudinary.com') ? 'blur' : 'empty'}
                        blurDataURL={imageUrl.includes('cloudinary.com') ? getPlaceholderUrl(imageUrl) : undefined}
                      />
                    ) : (
